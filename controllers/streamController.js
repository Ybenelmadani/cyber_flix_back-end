...
const getEpisodeParams = (query = {}) => ({
  seasonNumber: pickFirstDefined(
    query.seasonNumber,
    query.season,
    query.s
  ),
  episodeNumber: pickFirstDefined(
    query.episodeNumber,
    query.episode,
    query.e
  ),
...
const buildAutoEmbedSource = ({ mediaType } = {}) => ({
...
const buildVirtualEmbedStream = ({
  ...
  return {
    ...
    title: "",
    poster: "",
    sources: [buildAutoEmbedSource({ mediaType })],
...
const buildAutoSubtitleCandidates = ({
  mediaType,
  tmdbId,
  seasonNumber = null,
  episodeNumber = null,
  ...
  if (!normalizedTmdbId) return [];

  if (mediaType === "movie") {
    return [
      `/subtitles/movie/${normalizedTmdbId}/ar.vtt`,
      `/subtitles/${normalizedTmdbId}/ar.vtt`,
    ];
  }
  ...
  return [
    `/subtitles/tv/${normalizedTmdbId}/season-${season}/episode-${episode}/ar.vtt`,
    `/subtitles/tv/${normalizedTmdbId}/S${seasonPadded}E${episodePadded}/ar.vtt`,
    `/subtitles/${normalizedTmdbId}/S${seasonPadded}E${episodePadded}/ar.vtt`,
  ];
};

const buildSubtitleTracks = (stream, context = {}) => {
  const customArabicUrl = String(stream?.subtitleArabicUrl || "").trim();
  const candidates = customArabicUrl
    ? [customArabicUrl]
    : buildAutoSubtitleCandidates(context);

  return candidates.filter(Boolean).map((src, index) => ({
    id: `ar-${index}`,
    label: "Arabic",
    src,
    srcLang: "ar",
    kind: "subtitles",
    default: index === 0,
    isAutoGenerated: !customArabicUrl,
  }));
};

const sanitizeSource = (source, index, subtitleTracks = []) => ({
  id: String(index),
  name:
    source.name ||
...
const enrichSourceForPlayback = (source, context = {}) => {
  ...
  return {
    ...
    subtitles: buildSubtitleTracks(context.stream, context),
  };
};
...
exports.getStreamByTmdb = async (req, res) => {
  try {
    ...
    const { seasonNumber, episodeNumber } = getEpisodeParams(req.query);
    ...
    const subtitleTracks = buildSubtitleTracks(stream, {
      mediaType,
      tmdbId,
      seasonNumber,
      episodeNumber,
    ...
    return res.json({
      ...
      stream: {
        ...stream,
        sources: filteredSources.map((source, index) =>
          sanitizeSource(source, index, subtitleTracks)
        ),
      },
    });
  } catch (error) {
...
exports.createStream = async (req, res) => {
  try {
    ...
    const payload = {
      ...
      title,
      poster,
      subtitleArabicUrl: String(subtitleArabicUrl || "").trim(),
      sources,
    };
...
exports.updateStream = async (req, res) => {
  try {
    ...
    if (subtitleArabicUrl !== undefined) {
      stream.subtitleArabicUrl = String(subtitleArabicUrl || "").trim();
    }
